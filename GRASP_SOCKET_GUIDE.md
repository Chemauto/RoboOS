# 抓取模块 Socket 通信指南

## 📋 方案说明

由于抓取脚本需要在开发板上打开摄像头显示画面，使用SSH执行会有问题。因此改用**Socket通信**方案：

```
你的电脑 (RoboOS)                    开发板
     |                                    |
     |  发送 0xAA 指令                    |
     |----------------------------------->|
     |                                    |  1. 接收 0xAA
     |                                    |  2. 打开摄像头
     |                                    |  3. 显示画面（在开发板上）
     |                                    |  4. 执行抓取脚本
     |                                    |  5. 返回结果
     |<-----------------------------------|
     |  收到响应                           |
```

**协议特点：**
- ✅ 超简单：只需发送单字节 `0xAA`
- ✅ 不需要参数：不区分颜色，统一处理
- ✅ 容易调试：十六进制一目了然
- ✅ 快速响应：数据量小，解析快

---

## 🚀 快速开始（3步）

### 步骤1：复制服务器程序到开发板

```bash
# 复制服务器程序到开发板
scp grasp_server.py HwHiAiUser@192.168.0.155:/home/HwHiAiUser/

# 会要求输入密码
```

### 步骤2：在开发板上启动服务器

```bash
# SSH登录到开发板
ssh HwHiAiUser@192.168.0.155

# 进入主目录
cd ~

# 启动服务器（前台运行，可以看到日志）
python3 grasp_server.py
```

你会看到：
```
============================================================
抓取服务器启动成功
监听地址: 0.0.0.0:9999
抓取指令: 0xAA
抓取脚本: /home/HwHiAiUser/test.sh
============================================================
等待客户端连接...
按 Ctrl+C 停止服务器
============================================================
```

### 步骤3：重启 RoboOS 并测试

```bash
# 在你的电脑上，重启 master 和 slaver
# 按 Ctrl+C 停止当前的，然后：

# 终端2：重启 Master
python master/run.py

# 终端3：重启 Slaver
python slaver/run.py
```

通过 Web UI 或测试脚本发送：
```
"执行抓取"
```

---

## 📡 通信协议

### 指令格式

```
0xAA  (单字节)
```

**说明：**
- 发送单个字节 `0xAA`（十六进制）= `170`（十进制）
- 开发板收到后执行抓取脚本
- 不需要任何参数

### 响应格式

```
SUCCESS:00:抓取执行成功
FAILED:04:抓取失败
```

### 响应码

| 响应码 | 说明 |
|--------|------|
| 0x00 | 执行成功 |
| 0x01 | 指令格式错误（不是 0xAA） |
| 0x02 | （保留） |
| 0x03 | 抓取脚本不存在 |
| 0x04 | 抓取失败 |
| 0x05 | 未知错误 |

---

## 🔧 后台运行服务器（推荐）

### 方法1：使用 nohup

```bash
# SSH登录到开发板
ssh HwHiAiUser@192.168.0.155

# 后台运行服务器
cd ~
nohup python3 grasp_server.py > grasp_server.log 2>&1 &

# 查看日志
tail -f grasp_server.log

# 停止服务器
ps aux | grep grasp_server
kill <PID>
```

### 方法2：使用 systemd（最佳）

创建服务文件 `/etc/systemd/system/grasp-server.service`：

```ini
[Unit]
Description=Grasp Server for RoboOS
After=network.target

[Service]
Type=simple
User=HwHiAiUser
WorkingDirectory=/home/HwHiAiUser
ExecStart=/usr/bin/python3 /home/HwHiAiUser/grasp_server.py
Restart=on-failure
RestartSec=5

[Install]
WantedBy=multi-user.target
```

安装并启动服务：

```bash
# 复制服务文件到开发板
scp grasp-server.service HwHiAiUser@192.168.0.155:/tmp/

# SSH登录到开发板
ssh HwHiAiUser@192.168.0.155

# 安装服务（需要root权限）
sudo mv /tmp/grasp-server.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable grasp-server
sudo systemctl start grasp-server

# 查看状态
sudo systemctl status grasp-server

# 查看日志
sudo journalctl -u grasp-server -f
```

---

## 🧪 测试连接

### 测试1：检查端口是否监听

```bash
# 在你的电脑上测试
telnet 192.168.0.155 9999

# 或使用 nc
nc -zv 192.168.0.155 9999
```

### 测试2：手动发送指令

**使用 Python：**
```python
import socket
sock = socket.socket()
sock.connect(('192.168.0.155', 9999))
sock.send(bytes([0xAA]))  # 发送 0xAA
response = sock.recv(1024)
print(response.decode())
sock.close()
```

**使用 echo + nc：**
```bash
# 注意：echo 可能不适用于发送二进制数据
# 推荐 Python 测试
```

---

## 📝 开发板脚本要求

你的抓取脚本 (`test.sh`) 只需要是一个可执行的bash脚本：

```bash
#!/bin/bash

# 你的抓取程序
echo "启动抓取程序..."

# 示例：运行 Python 抓取脚本
python3 your_grasp_program.py

echo "抓取完成"
```

**重要提示：**
- 脚本必须可执行：`chmod +x test.sh`
- 脚本可以打开摄像头显示
- 不需要接收颜色参数（已固定在脚本中）
- 脚本路径：`/home/HwHiAiUser/test.sh`

---

## 💬 使用示例

### 通过 Web UI

```
"执行抓取"
"启动抓取功能"
"抓取物体"
```

### 通过测试脚本

```bash
python test_robot.py

# 选择 "3. 所有模块"
# 选择 "3. 自定义任务"
# 输入："执行抓取"
```

### 复合任务示例

```
"移动到篮子位置，然后执行抓取"
"机械臂复位，执行抓取，然后回到入口"
"导航到厨房桌子，执行抓取"
```

---

## ❓ 常见问题

### Q1: 连接被拒绝

**问题：** Connection refused

**解决：**
1. 确认开发板服务器正在运行：`ps aux | grep grasp_server`
2. 检查端口：`netstat -tlnp | grep 9999`
3. 检查网络连接：`ping 192.168.0.155`

### Q2: 服务器启动失败

**问题：** Address already in use

**解决：**
```bash
# 查找占用端口的进程
sudo lsof -i :9999

# 杀死进程
kill <PID>

# 或修改服务器程序使用其他端口
```

### Q3: 脚本不存在

**问题：** FAILED:03:抓取脚本不存在

**解决：**
1. 确认脚本路径：`ls -l /home/HwHiAiUser/test.sh`
2. 添加执行权限：`chmod +x /home/HwHiAiUser/test.sh`
3. 修改 `grasp_server.py` 中的 `SCRIPT_PATH`

### Q4: 抓取超时

**问题：** 抓取超时（30秒）

**解决：**
1. 修改 `grasp_server.py` 中的超时时间（第47行）
2. 优化抓取脚本速度
3. 检查是否有弹窗或交互式输入

### Q5: 收到空数据

**问题：** 服务器收到空数据

**解决：**
1. 重启 slaver 加载新的 grasp 模块
2. 检查 RoboOS 配置：`BOARD_CONFIG["socket_host"]`
3. 确认发送的是单字节 `0xAA`

---

## 📊 日志查看

### 开发板端

```bash
# 前台运行时直接在终端查看
python3 grasp_server.py

# 后台运行时查看日志文件
tail -f grasp_server.log

# 使用 systemd 时
sudo journalctl -u grasp-server -f
```

### RoboOS 端

```bash
# 查看 slaver 日志
tail -f slaver/.log/agent.log | grep grasp

# 搜索抓取相关日志
grep "grasp" slaver/.log/agent.log
```

**正常日志示例：**
```
[grasp.grasp_object] 开始执行抓取
[grasp._send_via_socket] 连接到开发板: 192.168.0.155:9999
[grasp._send_via_socket] 连接成功
[grasp._send_via_socket] 发送指令: 0xAA
[grasp._send_via_socket] 收到响应: SUCCESS:00:抓取执行成功
```

---

## 🎯 完整工作流程

1. **准备开发板**
   - 安装抓取模型和脚本到 `/home/HwHiAiUser/test.sh`
   - 确保摄像头可用
   - 添加执行权限：`chmod +x test.sh`

2. **启动服务器**
   ```bash
   ssh HwHiAiUser@192.168.0.155
   cd ~
   python3 grasp_server.py
   ```

3. **启动 RoboOS**
   ```bash
   # 重启 master 和 slaver
   python master/run.py
   python slaver/run.py
   ```

4. **发送抓取任务**
   ```
   "执行抓取"
   ```

5. **观察执行**
   - 开发板上显示摄像头画面
   - 执行抓取动作
   - 返回结果到 RoboOS

---

## 🔄 配置更新

### RoboOS 端配置

`slaver/demo_robot_local/grasp.py`:
```python
BOARD_CONFIG = {
    "connection_type": "socket",       # 使用 Socket
    "socket_host": "192.168.0.155",  # 开发板IP
    "socket_port": 9999,              # 端口
    "socket_timeout": 60,              # 超时时间（秒）- 根据抓取脚本执行时间调整
}
```

### 开发板端配置

`grasp_server.py`:
```python
HOST = '0.0.0.0'                          # 监听所有接口
PORT = 9999                              # 端口
SCRIPT_PATH = '/home/HwHiAiUser/test.sh'  # 脚本路径
GRASP_COMMAND = 0xAA                      # 抓取指令
```

---

## 📖 代码修改说明

### 旧协议（已废弃）

```
发送: "GRASP:红色\n"  ← 文本协议
需要: 颜色参数
解析: 复杂
```

### 新协议（当前）

```
发送: 0xAA           ← 字节协议
需要: 无参数
解析: 超简单
```

---

## 🎉 优势

✅ **协议简单** - 单字节 0xAA
✅ **不需要参数** - 统一处理
✅ **易于调试** - 十六进制清晰
✅ **快速响应** - 数据量小
✅ **显示正常** - 摄像头画面在开发板上
✅ **无需认证** - Socket直连
✅ **持续监听** - 服务器一直运行

---

## 📚 相关文档

- [MODULE_DEVELOPMENT_GUIDE.md](MODULE_DEVELOPMENT_GUIDE.md) - 完整开发指南
- [README.md](README.md) - 快速开始

---

现在可以开始使用了！🚀
