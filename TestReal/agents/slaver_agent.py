...
# 从 TestReal.agents 导入 models
from TestReal.agents.models import ChatMessage
from flag_scale.flagscale.agent.collaboration import Collaborator
from mcp import ClientSession
from rich.panel import Panel
from rich.text import Text
# 从 TestReal.tools 导入 memory 和 monitoring
from TestReal.tools.memory import ActionStep, AgentMemory, SceneMemory
from TestReal.tools.monitoring import AgentLogger, LogLevel, Monitor
...
class ToolCallingAgent(MultiStepAgent):
    ...
    async def _execute_tool_call(
        ...
        # 检查 observation 的类型，它可能是一个 ChatMessage 对象
        if isinstance(observation, ChatMessage) and observation.content:
            observation = observation.content
        elif hasattr(observation, 'text'): # Fallback for some structures
            observation = observation.text
        else:
            observation = str(observation) # Fallback to string conversion
