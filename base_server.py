...
# 自动查找并添加 RealBase 到路径
def find_realbase_path():
    """查找 RealBase 目录"""
    # 可能的 RealBase 位置
    possible_paths = [
        os.path.join(os.path.dirname(__file__), 'RealBase'),
        os.path.join(os.path.dirname(__file__), '../RealBase'),
        os.path.expanduser('~/RealBase'),
        '/home/HwHiAiUser/RealBase',
        '/home/HwHiAiUser/RoboOS/RealBase',
        os.path.abspath('./RealBase'),
    ]
    
    for path in possible_paths:
        if os.path.exists(os.path.join(path, 'motor_controller.py')):
            print(f"[路径] 找到 RealBase: {path}")
            return os.path.abspath(path)
    
    print("[警告] 未找到 RealBase 目录，请确保 RealBase 已复制到开发板上")
    print("[警告] 尝试的位置：")
    for path in possible_paths:
        print(f"  - {path}")
    return None
...
# 查找并添加 RealBase 到路径
realbase_path = find_realbase_path()
if realbase_path:
    sys.path.insert(0, realbase_path)
else:
    print("[错误] 无法找到 RealBase 目录，程序退出")
    print("[提示] 请运行以下命令复制 RealBase:")
    print("  scp -r /home/dora/RoboOs/LekiwiTest/RealBase HwHiAiUser@192.168.0.155:/home/HwHiAiUser/")
    sys.exit(1)
...
try:
    ...
    print(f"[提示] RealBase 路径: {realbase_path}")
    print(f"[提示] 当前路径: {os.getcwd()}")
    print(f"[提示] sys.path: {sys.path}")
    sys.exit(1)
